...
-C contig_to_bin_map : falcultative argument, allow to add information regarding vertexes in your graph, namely membership of a contig in a bin. The main interest of this option is that Parse_gfa will add the bin membership information as a color in the .gfa file in a way that bandage can understand allowing to display effectively such membership. The file contig_to_bin_map need to be in the following format : name_of_contig:name_of_bin. Name_of_contig need to be the same as in file_in. At the moment a maximum of 20 differents bin is supported, for coloring reason. 
